...
**Mount management** — Volumes can optionally declare a `mount` section to automate the mount/umount lifecycle via [udisks2](https://www.freedesktop.org/wiki/Software/udisks/). For mount-managed volumes, `path` is **optional**: declare it (backed by an `/etc/fstab` entry) to mount at a fixed path, or omit it to let udisks mount at `/run/media/<user>/<label>` — nbkp discovers that path at runtime. There is no `strategy` field (udisks is the sole backend) and encryption no longer takes a `mapper-name` (udisks names the unlocked device itself — `luks-<uuid>`, or the LUKS2 header label, or a crypttab name — and nbkp discovers whichever it chose). See [Volume Mount Management](./internals.md#volume-mount-management) and the [usage guide](./usage.md#mount-management-with-udisks2) for details.
...
| `path` | string | `null` | Absolute path to the volume. `~` is expanded to the user's home directory. Trailing slashes are stripped. Optional only for mount-managed volumes: when omitted, the mountpoint udisks chooses (``/run/media/<user>/<label>``) is discovered at runtime. Required when ``mount`` is not set. |
...
| `path` | string | `null` | Absolute path on the remote host. Trailing slashes are stripped. `~` is not expanded (it refers to the remote user's home and is resolved by SSH/rsync). Optional only for mount-managed volumes: when omitted, the mountpoint udisks chooses (``/run/media/<user>/<label>``) is discovered at runtime. Required when ``mount`` is not set. |
| `mount` | object | `null` | Mount management config. When set, nbkp manages the volume's mount lifecycle. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `device-uuid` | string | **required** | Device UUID for drive detection (from ``/dev/disk/by-uuid/``). For encrypted volumes: the LUKS container UUID (from crypttab/blkid); the unlocked device is named ``luks-<uuid>`` by udisks unless the LUKS2 header carries a label or a crypttab entry overrides it (nbkp discovers the actual name either way). For unencrypted volumes: the filesystem UUID (from fstab/blkid). |
