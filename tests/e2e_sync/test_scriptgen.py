"""E2E tests: generated script runs end-to-end."""
...
class TestGeneratedScriptRun:
    def test_chain_config_runs_successfully(
        self, tmp_path: Path
    ) -> None:
        """Generated script from a chain config runs end-to-end."""
    ...
    @_requires_bash4
    def test_chain_config_runs_no_portable(
        self, tmp_path: Path
    ) -> None:
        """--no-portable script runs end-to-end (bash 4+)."""
        config = _build_chain_config(tmp_path)
        create_seed_sentinels(config)
        seed_volume(config.volumes["src"])
